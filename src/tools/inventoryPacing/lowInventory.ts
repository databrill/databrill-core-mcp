 ...
 * optional: absent relations/columns use a fallback; other database errors fail the Effect.
 *
...
export function loadLowInventory(
	sql: postgres.Sql,
	stores: ResolvedStore[],
	velocityDaysRaw: number,
): Effect.Effect<LowInventoryRow[], Error> {
	return Effect.gen(function* () {
		...
		for (const store of stores) {
			...
			const [stMaxRow] = yield* runCompiled(
				sql,
				() =>
					db
			...
			if (stEnd >= day28start) {
				const stRows = yield* runCompiled(
					sql,
					() =>
						db
							.selectFrom("amzreport_SALES_AND_TRAFFIC__skuByDay")
			...
			if (aoStart <= yesterday) {
				const aoRows = yield* runCompiled(
					sql,
					() =>
						db
							...
							.select((eb) => [
								...
								eb.cast<number | null>(
									eb.fn.sum<number | null>(
										eb.case().when("localdate", "=", isoDateParam(yesterday)).then(
											eb.ref("quantity"),
										)
											.else(ksql.lit(0)).end(),
									),
								...
								eb.cast<number | null>(
									eb.fn.sum<number | null>(
										eb.case().when("localdate", ">=", isoDateParam(day7start)).then(
											eb.ref("quantity"),
										)
											.else(ksql.lit(0)).end(),
									),
								...
								eb.cast<number | null>(eb.fn.sum<number | null>("quantity"), "integer").as(
									"units28d",
								),
							])
			...
			yield* Effect.gen(function* () {
				const catalogRows = yield* runCompiled(
					sql,
					() =>
						db
			...
			}).pipe(Effect.catchIf(isMissingRelationOrColumn, () => Effect.void /* best-effort */));
			const missingParent = topAsins.filter((a) => !parentAsinMap.has(a));
			if (missingParent.length > 0) {
				yield* Effect.gen(function* () {
					const stParentRows = yield* runCompiled(
						sql,
						() =>
							db
								.selectFrom("amzreport_SALES_AND_TRAFFIC__skuByDay")
			...
			const fbaRows = yield* runCompiled(
				sql,
				() =>
					db
			...
			yield* Effect.gen(function* () {
				const fbmRows = yield* runCompiled(
					sql,
					() =>
						db
							.selectFrom((eb) =>
								eb
									...
									.where((eb2) =>
										eb2(jsonbText(eb2.ref("doc"), "fulfillment-channel"), "=", "DEFAULT")
									)
									.where("marketplaceId", "=", marketplaceId)
									.where((eb2) =>
							...
							.select((eb) => [
								"asin",
								eb.cast<number | null>(eb.fn.sum<number | null>("skuQty"), "integer").as(
									"inventoryFbm",
								),
							])
							.groupBy("asin")
			...
			yield* Effect.gen(function* () {
				const familyRows = yield* runCompiled(
					sql,
					() =>
						db
			...
			}).pipe(Effect.catchIf(isMissingRelationOrColumn, () =>
				Effect.sync(() => {
